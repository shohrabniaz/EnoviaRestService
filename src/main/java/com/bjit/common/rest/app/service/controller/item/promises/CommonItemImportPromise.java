 ...
 * @author BJIT
 */
public class CommonItemImportPromise implements ItemOrBOMImport {

    private static final org.apache.log4j.Logger COMMON_MAN_ITEM_LOGGER = org.apache.log4j.Logger.getLogger(CommonItemImportPromise.class);
    ...
    public <T, K> K doImport(final Context context, final T objectDataBeanRequest) {
        ...
        dataTreeList.forEach((DataTree dataTree) -> {
            ...
            try {
                ...
                responseMessageFormatterBean.setRowSequenceIdentifier(dataTree.getRowSequenceIdentifier());
                String itemType = createObjectBean.getTnr().getType();

                Callable commonItemImportProcess = getItemImportFacade(context, createObjectBean, responseMessageFormatterBean, objectDataBean);
    ...
    protected Callable getItemImportFacade(final Context context, CreateObjectBean createObjectBean, ResponseMessageFormaterBean responseMessageFormatterBean, ObjectDataBean objectDataBean) {
        Callable commonItemImportProcess = new CommonItemImportFacade(context, createObjectBean, responseMessageFormatterBean, objectDataBean.getSource());
