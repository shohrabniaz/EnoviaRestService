package com.bjit.common.rest.app.service.comosData.xmlPreparation.equipmentProceessors.constants;

import org.springframework.stereotype.Component;

@Component
public class HVACLineConstants implements IConstantType {
    public String TYPE = "HVAC_Line";
    public String DISCIPLINE = "HVAC_Line";
    public String INSTANCE_TYPE = "HVAC_Line_Inst";
    public String INSTANCE_DISCIPLINE = "HVAC_Line_Inst";

    @Override
    public String getConstantType() {
        return "HVACLineConstants";
    }

    @Override
    public void setConstants(String type, String discipline, String instanceType, String instanceDiscipline) {
        this.TYPE = type;
        this.DISCIPLINE = discipline;
        this.INSTANCE_TYPE = instanceType;
        this.INSTANCE_DISCIPLINE = instanceDiscipline;
    }
}
