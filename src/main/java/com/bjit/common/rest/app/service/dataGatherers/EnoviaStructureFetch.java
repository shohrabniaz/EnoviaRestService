 ...
 * @author Tahmid
 */
public class EnoviaStructureFetch {
    private static final Logger ENV_STRUCTURE_LOGGER = Logger.getLogger(EnoviaStructureFetch.class);
    public static HashMap<String, Integer> bomIndexTracker;

    public static Object getEnoviaStructure(String type, String name, String rev, String expandLevel, String attributesAsString, String drawingType) {
        ...
        try {
            ...
            if (enoviaBOMResponse.getStatusCode() == HttpStatus.OK) {
                ...
                enoviaRootItem = createEnoviaStructure(resultArray, enoviaRootItem);
            } else {
    ...
    public static EnoviaItem createEnoviaStructure(JsonArray bomArray, EnoviaItem enoviaItem) {
        ...
        if (bomIndexTracker.get(key) != null) {
            ...
            itemNode.getAsJsonArray("bomLines").forEach(item -> {
                ...
                createEnoviaStructure(bomArray, childItem);
            });
        }
    ...
    private static String roundQuantityWithPrecision(String value) {
        if (!NullOrEmptyChecker.isNullOrEmpty(value)) {
            try {
                ...
                Integer precisionPoint = Integer.parseInt(PropertyReader.getProperty("bom.compare.enovia.quantity.precision"));
