...
//@Scope("prototype")
@RequestScope
public class EquipmentCategoryFactory implements IRFLPDataFactory, IComosItemTypeUtils, IComosLogicalXMLDataFactory, IComosLogicalInstanceDataFactory {
    ...
    public RFLVPMItem getLogicalData(EquipmentChild item) {
        try {
            ...
            String description = PropertyReader.getProperty("comos.equipment.category.description");
...
//                    Optional.ofNullable(Optional.ofNullable(item.getAttributes()).orElse(new HashMap<>()).get(description)).orElse(""))));
    ...
    private HashMap<String, HashMap<String, String>> getItemSourceDestinationData(String type) {
        try {
            HashMap<String, HashMap<String, String>> itemAttributes = mapper.getConfigurableMap().get("EquipmentCategory").get(type).get("item");
            return itemAttributes;
        } catch (NullPointerException exp) {
            String exceptionMessage = type + " has not been configured as a map under EquipmentCategory in the Comos XML map";
    ...
    private HashMap<String, HashMap<String, String>> getRelationSourceDestinationData(String type) {
        try {
            HashMap<String, HashMap<String, String>> itemAttributes = mapper.getConfigurableMap().get("EquipmentCategory").get(type).get("relation");
            return itemAttributes;
        } catch (NullPointerException exp) {
            String exceptionMessage = type + " has not been configured as a map under EquipmentCategory in the Comos XML map";
            log.error(exceptionMessage);
    ...
    public String getType() {
        return PropertyReader.getProperty("comos.equipment.category.factory.type");
    }

    @Override
    public String getPrefix() {
//        return "C1";
        return PropertyReader.getProperty("comos.equipment.category.factory.prefix");
    }

    @Override
    public Integer getLevel() {
        return Integer.parseInt(PropertyReader.getProperty("comos.equipment.category.factory.level"));
    }
