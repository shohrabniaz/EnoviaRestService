tcl;
eval {
	set target_jpos(1) "emxCustomRevisionTrigger"
	...
	proc addProgram { name fp_LogFile } {
		set fileNameAdditional "_mxJPO.java"
		set file_name [ concat $name$fileNameAdditional ]
	...
	set iRetCode [ catch {
		foreach index [array names target_jpos] {
			backupProgram $target_jpos($index) $fp_LogFile
			addProgram $target_jpos($index) $fp_LogFile
			compileProgram $target_jpos($index) $fp_LogFile
			verifyJPO $target_jpos($index) $fp_LogFile $fp_err_LogFile
		}
