...
eval {
	...
	set triggerType [ split [ mql temp query bus "eService Trigger Program Parameters" "emxMastershipChangeRevisionTrigger" "MastershipRevision" select id dump ] , ]
	...
	mql add bus "eService Trigger Program Parameters" "emxMastershipChangeRevisionTrigger"  "MastershipRevision"\
	...
	"eService Program Argument 7" "\${NEWOBJECTID}"\
	"eService Program Argument 8" "\${NEWREV}"\
	"eService Program Name" "emxCustomRevisionTrigger"\
	"eService Constructor Arguments" "\${NEWOBJECTID} \${STATENAME} \${NEXTSTATE} \${NAME} \${NEWREVISION} \${TYPE} \${NEWOBJECTID} \${NEWREV}"\
	...
	add trigger majorrevision action emxTriggerManagerBase input "emxMastershipChangeRevisionTrigger";
