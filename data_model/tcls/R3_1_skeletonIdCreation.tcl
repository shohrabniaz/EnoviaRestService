...
# from an external file named "R3_1_skeletonIdAttributeUpgradation.txt"
# @Version: 1.0
# @Created By: BJIT Ltd.
# @Release: 
#########################################################################################################################
...
eval {
	...
	proc createSkeletonIdAndUpdateAttr { fp_SkeletonIdFile fp_LogFile } {
		...
		set type_attr_list [open "R3_1_skeletonIdAttributeUpgradation.txt" "r"]
		set skeletonId ""
		...
		while { [eof $type_attr_list] != 1 } {
			...
			if { [string compare $attrName "Type"] == 0 } {
				...
				set listOfExistingInterfaces [ split [ mql print bus $skeletonId select interface dump | ] "|" ]
				foreach interfaceName $listOfInterfaces {
					set interfaceIsExist "false"
					foreach existingInterfaceName $listOfExistingInterfaces {
						if { [ string compare $interfaceName $existingInterfaceName ] == 0 } {
							set interfaceIsExist "true"
						}
					}
					if { [ string compare $interfaceIsExist "false" ] == 0 } {
			...
			} elseif { [string compare $attrName "RemoveInterfaceList"] == 0 } {
				set listOfRemoveInterfaces [split $attrValue ","]
				
				set listOfExistingInterfaces [ split [ mql print bus $skeletonId select interface dump | ] "|" ]
				foreach removeInterfaceName $listOfRemoveInterfaces {
					foreach existingInterfaceName $listOfExistingInterfaces {
						if { [ string compare $removeInterfaceName $existingInterfaceName ] == 0 } {
							writeLog $fp_LogFile "#################################################>>> Remove Interface $removeInterfaceName <<<#################################################"
							writeLog $fp_LogFile "Removing existing (Length/Mass/Volume/Area) interface : $removeInterfaceName"
							set modify_command "mql modify bus $skeletonId remove interface $removeInterfaceName"
							processMQLModifyCommand $fp_LogFile $modify_command			
						}
					}
				}
